...
angular.module(module.exports, []).controller('EditCtrl', [
  'currentUser',
  'game',
  '$game',
  '$q',
  ...
  function(currentUser, game, $game, $q, $state) {
    ...
    this.save = function() {
      ...
      this.pendingDeletion.forEach(function(participantId) {
        deletePromises.push($game.removeParticipant(game.id, participantId));
      });
      ...
      this.pendingExclusions.forEach(function(exclusionItem) {
        exclusionPromises.push(
          $game.setExclusions(
            game.id,
            exclusionItem.participant,
            exclusionItem.exclusions
          )
        );
      });

...
angular
  .module(moduleName, [require('../services/game')])
  .controller('ViewOpenCtrl', [
    ...
    function(
      ...
      if (this.isParticipant()) {
        this.likes = _.find(this.game.participants, {
          id: currentUser.id
        }).santa.likes;
        this.dislikes = _.find(this.game.participants, {
          id: currentUser.id
        }).santa.dislikes;
      }
      ...
      this.closeGame = function() {
        return $game.setStatus(game.id, 'closed').then(function(result) {
          $state.go($state.current, $stateParams, {reload: true});
        });
      };
    }
  ])
  .controller('ViewClosedCtrl', [
    '$facebook',
    '$game',
    '$q',
    'game',
    function($facebook, $game, $q, game) {
      ...
      $facebook
        ...
        .then(function(recipient) {
          return $facebook.getPicture(recipient.id, 'width(320)');
        })
        .then(
          function(picture) {
